IF DB_ID(N'FourBytsFood') IS NULL
BEGIN
    CREATE DATABASE FourBytsFood;
END;
GO
...
BEGIN
    UPDATE Empresas
       SET FantasyName = N'4Byts Food',
           CorporateName = N'4Byts Food',
           ...
           EmailSmtpFromName = CASE WHEN EmailSmtpFromName = N'' THEN N'4Byts Food' ELSE EmailSmtpFromName END
...
BEGIN
    ...
    VALUES
        (N'empresa-principal', N'4Byts Food', N'4Byts Food', N'06.332.765/0001-05',
...
-- Módulo exclusivo da edição 4Byts Food: salão, mesas e comandas.
IF OBJECT_ID(N'RestaurantTables', N'U') IS NULL
BEGIN
    CREATE TABLE RestaurantTables
    (
        Id NVARCHAR(40) NOT NULL CONSTRAINT PK_RestaurantTables PRIMARY KEY,
        CompanyId NVARCHAR(40) NOT NULL,
        Name NVARCHAR(80) NOT NULL,
        Capacity INT NOT NULL CONSTRAINT DF_RestaurantTables_Capacity DEFAULT 4,
        Status NVARCHAR(20) NOT NULL CONSTRAINT DF_RestaurantTables_Status DEFAULT N'available',
        CreatedAt DATETIMEOFFSET NOT NULL CONSTRAINT DF_RestaurantTables_CreatedAt DEFAULT SYSDATETIMEOFFSET(),
        UpdatedAt DATETIMEOFFSET NOT NULL CONSTRAINT DF_RestaurantTables_UpdatedAt DEFAULT SYSDATETIMEOFFSET(),
        CONSTRAINT CK_RestaurantTables_Capacity CHECK (Capacity BETWEEN 1 AND 100),
        CONSTRAINT CK_RestaurantTables_Status CHECK (Status IN (N'available', N'occupied', N'reserved', N'inactive')),
        CONSTRAINT UQ_RestaurantTables_Company_Name UNIQUE (CompanyId, Name)
    );
    CREATE INDEX IX_RestaurantTables_Company_Status ON RestaurantTables (CompanyId, Status);
END;

IF OBJECT_ID(N'RestaurantTabs', N'U') IS NULL
BEGIN
    CREATE TABLE RestaurantTabs
    (
        Id NVARCHAR(40) NOT NULL CONSTRAINT PK_RestaurantTabs PRIMARY KEY,
        CompanyId NVARCHAR(40) NOT NULL,
        TableId NVARCHAR(40) NULL,
        Number INT NOT NULL,
        CustomerName NVARCHAR(180) NOT NULL CONSTRAINT DF_RestaurantTabs_CustomerName DEFAULT N'Consumidor',
        GuestCount INT NOT NULL CONSTRAINT DF_RestaurantTabs_GuestCount DEFAULT 1,
        Status NVARCHAR(20) NOT NULL CONSTRAINT DF_RestaurantTabs_Status DEFAULT N'open',
        Notes NVARCHAR(500) NOT NULL CONSTRAINT DF_RestaurantTabs_Notes DEFAULT N'',
        OpenedById NVARCHAR(40) NOT NULL,
        OpenedByName NVARCHAR(180) NOT NULL,
        OpenedAt DATETIMEOFFSET NOT NULL CONSTRAINT DF_RestaurantTabs_OpenedAt DEFAULT SYSDATETIMEOFFSET(),
        ClosedAt DATETIMEOFFSET NULL,
        CONSTRAINT FK_RestaurantTabs_Table FOREIGN KEY (TableId) REFERENCES RestaurantTables (Id),
        CONSTRAINT CK_RestaurantTabs_GuestCount CHECK (GuestCount BETWEEN 1 AND 999),
        CONSTRAINT CK_RestaurantTabs_Status CHECK (Status IN (N'open', N'closed', N'cancelled')),
        CONSTRAINT UQ_RestaurantTabs_Company_Number UNIQUE (CompanyId, Number)
    );
    CREATE INDEX IX_RestaurantTabs_Company_Status ON RestaurantTabs (CompanyId, Status);
    CREATE INDEX IX_RestaurantTabs_Table_Status ON RestaurantTabs (TableId, Status);
END;

IF OBJECT_ID(N'RestaurantTabItems', N'U') IS NULL
BEGIN
    CREATE TABLE RestaurantTabItems
    (
        Id NVARCHAR(40) NOT NULL CONSTRAINT PK_RestaurantTabItems PRIMARY KEY,
        CompanyId NVARCHAR(40) NOT NULL,
        TabId NVARCHAR(40) NOT NULL,
        ProductId NVARCHAR(40) NULL,
        ProductName NVARCHAR(180) NOT NULL,
        Quantity DECIMAL(12,3) NOT NULL,
        UnitPrice DECIMAL(18,2) NOT NULL,
        Status NVARCHAR(20) NOT NULL CONSTRAINT DF_RestaurantTabItems_Status DEFAULT N'pending',
        Notes NVARCHAR(500) NOT NULL CONSTRAINT DF_RestaurantTabItems_Notes DEFAULT N'',
        CreatedAt DATETIMEOFFSET NOT NULL CONSTRAINT DF_RestaurantTabItems_CreatedAt DEFAULT SYSDATETIMEOFFSET(),
        UpdatedAt DATETIMEOFFSET NOT NULL CONSTRAINT DF_RestaurantTabItems_UpdatedAt DEFAULT SYSDATETIMEOFFSET(),
        CONSTRAINT FK_RestaurantTabItems_Tab FOREIGN KEY (TabId) REFERENCES RestaurantTabs (Id) ON DELETE CASCADE,
        CONSTRAINT FK_RestaurantTabItems_Product FOREIGN KEY (ProductId) REFERENCES Produtos (Id) ON DELETE SET NULL,
        CONSTRAINT CK_RestaurantTabItems_Quantity CHECK (Quantity > 0),
        CONSTRAINT CK_RestaurantTabItems_UnitPrice CHECK (UnitPrice >= 0),
        CONSTRAINT CK_RestaurantTabItems_Status CHECK (Status IN (N'pending', N'preparing', N'ready', N'delivered', N'cancelled'))
    );
    CREATE INDEX IX_RestaurantTabItems_Tab_Status ON RestaurantTabItems (TabId, Status);
END;

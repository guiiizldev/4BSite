    ...
    ILogger<AuthController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequest request)
    {
        ...
        try
        {
            centralLogin = await licenseService.AuthenticateAsync(request.Email, request.Password, HttpContext.RequestAborted);
        }
        ...
        if (centralLogin is { Success: false, InvalidCredentials: false })
            return StatusCode(StatusCodes.Status403Forbidden, new ApiResponse<object> { Success = false, Message = centralLogin.Error });
        ...
        {
            var statusCode = centralLogin?.Success == true ? StatusCodes.Status409Conflict
                : centralUnavailable ? StatusCodes.Status503ServiceUnavailable
                : result.LockedUntil is null ? StatusCodes.Status401Unauthorized : StatusCodes.Status429TooManyRequests;
            return StatusCode(statusCode, new ApiResponse<object>
            {
                Success = false,
                Message = centralUnavailable ? "A central 4Byts está temporariamente indisponível. Tente novamente em instantes." : result.Message,
